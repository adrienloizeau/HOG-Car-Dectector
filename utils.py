...
def create_bounding_boxes_df(boxes, frame_id="train/A_001.jpg"):
    """
    ...
    if not isinstance(boxes, np.ndarray):
        ...
        df_boxes = pd.DataFrame(data={"frame_id": frame_id, "bounding_boxes": bounding_boxes_str}, index=[0])
        return df_boxes
    else:
        # Convert array of boxes to list of strings
        bounding_boxes_str = []
        for box in boxes:
            bounding_boxes_str.append(' '.join(str(coord) for coord in box))
